  ...
  /** If you are using the standard ASCII input for the geometry
	...
	statframe->SetLineColor(kRed);
	    ...
	    if(std::fmod(f_design, 3) == 2) {
	      vol->AddNode(cf_plate, statnb * 1e6 + vnb * 1e5, new TGeoTranslation(0, 0, (vnb - 3. / 2.) * f_delta_z_view));
	    } else if(std::fmod(f_design, 3) == 0) {
	      vol->AddNode(cf_plate, statnb * 1e6 + vnb * 1e5, new TGeoTranslation(0, 0, (vnb - 3. / 2.) * f_delta_z_view - f_delta_z_layer / 2. - f_outer_straw_diameter / 2. - f_cf_thickness));
	      vol->AddNode(cf_plate, statnb * 1e6 + vnb * 1e5, new TGeoTranslation(0, 0, (vnb - 3. / 2.) * f_delta_z_view + f_delta_z_layer / 2. + f_outer_straw_diameter / 2. + f_cf_thickness));
	    }
		...
		if(std::fmod(f_design, 3) == 2) {
		  vol->AddNode(layerbox, statnb * 1e6 + vnb * 1e5 + lnb * 1e4, new TGeoTranslation(0, -floor_offset / 2., (vnb - 3. / 2.) * f_delta_z_view + (lnb - 1. / 2.) * (f_delta_z_layer + f_cf_thickness * 2)));
		} else {
